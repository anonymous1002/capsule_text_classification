...
def routing(u_hat_vecs, beta_a, iterations, output_capsule_num, i_activations):
    ...
    for i in range(iterations):
    	...
    	c = tf.split(leaky_routing, [1, output_capsule_num], 1)[1]
    	if i_activations is not None:
	    c = tf.transpose(tf.transpose(c, perm=[0,2,1]) * i_activations, perm=[0,2,1]) 
       	    outputs = squash_v1(K.batch_dot(c, u_hat_vecs, [2, 2]))
        if i < iterations - 1:
            b = b + K.batch_dot(outputs, u_hat_vecs, [2, 3])                                    
    poses = outputs 
    activations = K.sqrt(K.sum(K.square(poses), 2)) + beta_a
...
def capsules_init(inputs, shape, strides, padding, pose_shape, add_bias, name):
    with tf.variable_scope(name):   
        ...
        if False:                
            ...
            tf.logging.info("prim votes dimension:{}".format(u_hat_vecs.get_shape()))            
            ...
            activations = tf.reshape(activations, [
                        -1, poses_shape[1], poses_shape[2], shape[-1]]
                    )
        else:
        ...
        tf.logging.info("prim activations dimension:{}".format(activations.get_shape()))
...
#        
#        u_hat_vecs = vec_transformationByMat(
#                      poses,
...
#        i_activations = tf.sigmoid(slim.fully_connected(i_activations, input_pose_shape[1], scope='fc_activation', activation_fn=None))
...
def capsule_conv_layer(nets, shape, strides, iterations, name):   
    with tf.variable_scope(name):              
        ...
        if False:
            ...
            inputs_poses_patches = tf.tile(inputs_poses_patches,[1,1,shape[3],1])
            
            kernel = _get_weights_wrapper(
                  name='weights', shape=[1, shape[0]*shape[1]*shape[2], shape[3]], weights_decay_factor=0.0
                )
            kernel = tf.tile(kernel,[inputs_poses_shape[0]*inputs_poses_shape[1]*inputs_poses_shape[2],1,1])
            
            poses = tf.reduce_sum(tf.multiply(inputs_poses_patches, kernel[...,tf.newaxis]), axis=1)
            poses = tf.reshape(poses,[-1, inputs_poses_shape[1], inputs_poses_shape[2], shape[3], inputs_poses_shape[-1]])
            activations = K.sqrt(K.sum(K.square(poses), -1))
        else:
            u_hat_vecs = vec_transformationByConv(
...
#                      shared=True) 
            ...
            beta_v = _get_weights_wrapper(
                name='beta_b', shape=[shape[3], inputs_poses_shape[-1]]
                )    
#            i_activations_patches = tf.sigmoid(slim.fully_connected(i_activations_patches, shape[0]*shape[1]*shape[2], scope='fc_activation', activation_fn=None))            
